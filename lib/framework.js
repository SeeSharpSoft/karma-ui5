...
class Framework {
	...
	detectTypeFromFolder() {
		...
		const [hasWebapp, hasSrc, hasTest] = pathsExist([webappFolder, srcFolder, testFolder]);
	...
	init({config, logger}) {
		...
		if (this.config.ui5.type === "application") {
			...
			const [hasSrc, hasTest] = pathsExist([srcFolder, testFolder]);
			if (!hasSrc || !hasTest) {
		...
		this.middleware.init(this.config);
	...
	beforeMiddleware() {
		return this.middleware.beforeHandler;
	}
	middleware() {
